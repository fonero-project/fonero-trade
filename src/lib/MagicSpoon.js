...
const FoneroLedger = window.FoneroLedger;

// Spoonfed Fonero-SDK: Super easy to use higher level Fonero-Sdk functions
...
const MagicSpoon = {
  async Account(Server, keypair, opts, onUpdate) {
    ...
    sdkAccount.signWithLedger = (transaction) => {
      ...
      return new FoneroLedger.Api(new FoneroLedger.comm(Number.MAX_VALUE)).signTx_async(this.bip32Path, transaction).then((result) => {
        const signature = result.signature;
        const hint = keypair.signatureHint();
        const decorated = new FoneroSdk.xdr.DecoratedSignature({ hint, signature });
    ...
    sdkAccount.getFoneroBalance = () => sdkAccount.balances[sdkAccount.balances.length - 1].balance;

    // Expects FoneroSdk.Asset
    ...
    // Should always return at least one item (which is foneros)
    ...
    sdkAccount.explainReserve = () => {
      ...
      items.push({
        ...
        FNO: entriesTrustlines * 0.5,
      });

      items.push({
        entryType: 'Offers',
        amount: entriesOffers,
        FNO: entriesOffers * 0.5,
      });
      items.push({
        entryType: 'Others',
        amount: entriesOthers,
        FNO: entriesOthers * 0.5,
      });
      items.push({
        entryType: 'Extra',
        amount: '',
      ...
      const totalFoneros = _.sumBy(items, 'FNO');
      return {
        items,
        totalFoneros,
      };
    ...
    sdkAccount.maxFoneroSpend = () => {
      const balance = sdkAccount.getFoneroBalance();
  ...
  // opts.baseBuying -- FoneroSdk.Asset (example: FNO)
  // opts.counterSelling -- FoneroSdk.Asset (example: USD)
  ...
  buildTxCreateOffer(Server, spoonAccount, side, opts) {
    ...
    if (side === 'buy') {
      sdkBuying = opts.baseBuying; // ex: foneros
      ...
      sdkSelling = opts.baseBuying; // ex: foneros
    ...
    const transaction = new FoneroSdk.TransactionBuilder(spoonAccount)
      .addOperation(FoneroSdk.Operation.manageOffer(operationOpts));
      // DONT call .build()
  ...
  async buildTxSendPayment(Server, spoonAccount, opts) {
    ...
    let transaction = new FoneroSdk.TransactionBuilder(spoonAccount);
    try {
      const destAccount = await Server.loadAccount(opts.destination);
      transaction = transaction.addOperation(FoneroSdk.Operation.payment({
      ...
      if (!opts.asset.isNative()) {
        throw new Error('Destination account does not exist. To create it, you must send a minimum of 1 foneros to create it');
      }
      transaction = transaction.addOperation(FoneroSdk.Operation.createAccount({
        destination: opts.destination,
    ...
    if (opts.memo) {
      transaction = transaction.addMemo(Foneroify.memo(opts.memo.type, opts.memo.content));
    }
  ...
  buildTxSetInflation(spoonAccount, inflationDest) {
    let transaction = new FoneroSdk.TransactionBuilder(spoonAccount);
    transaction = transaction.addOperation(FoneroSdk.Operation.setOptions({
      inflationDest,
    }));
    // DONT call .build()
  ...
  buildTxChangeTrust(Server, spoonAccount, opts) {
    ...
    return new FoneroSdk.TransactionBuilder(spoonAccount)
      .addOperation(FoneroSdk.Operation.changeTrust(operationOpts));
      // DONT call .build()
  },
  buildTxRemoveOffer(Server, spoonAccount, offerId) {
    return new FoneroSdk.TransactionBuilder(spoonAccount)
      .addOperation(FoneroSdk.Operation.manageOffer({
        buying: FoneroSdk.Asset.native(),
        selling: new FoneroSdk.Asset('REMOVE', spoonAccount.accountId()),
        amount: '0',
        price: '1',
        offerId,
      }));
