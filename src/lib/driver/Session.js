...
export default function Send(driver) {
  ...
  this.pingLedger = (loop = false) => {
    ...
    new FoneroLedger.Api(new FoneroLedger.comm(4)).connect((success) => {
  ...
  this.handlers = {
    logInWithSecret: async (secretKey) => {
      const keypair = FoneroSdk.Keypair.fromSecret(secretKey);
      return this.handlers.logIn(keypair, {
        authType: 'secret',
      });
    },
    logInWithPublicKey: async (accountId) => {
      const keypair = FoneroSdk.Keypair.fromPublicKey(accountId);
      return this.handlers.logIn(keypair, {
        authType: 'pubkey',
      });
    },
    logInWithLedger: async (bip32Path) => {
      try {
        const connectionResult = await new FoneroLedger.Api(new FoneroLedger.comm(4)).getPublicKey_async(bip32Path);
        this.setupLedgerError = null;
        const keypair = FoneroSdk.Keypair.fromPublicKey(connectionResult.publicKey);
        return this.handlers.logIn(keypair, {
          authType: 'ledger',
          bip32Path,
        });
    ...
    // If you use sign, you have to pay attention to sequence numbers because js-fonero-sdk's .build() updates it magically
    ...
    sign: async (tx) => {
      if (this.account.inflation_destination === 'GDCHDRSDOBRMSUDKRE2C4U4KDLNEATJPIHHR2ORFL5BSD56G4DQXL4VW') {
        ...
        console.log(`https://www.fonero.org/laboratory/#txsigner?xdr=${encodeURIComponent(tx.toEnvelope().toXDR('base64'))}&network=public`);
      }
    ...
    buildSignSubmit: async (txBuilder) => {
      ...
      // This will also undo the sequence number that fonero-sdk magically adds
      let result = {
        status: 'cancel',
      };
    ...
    addTrust: async (code, issuer) => {
      ...
      // Having a "limit" is a design mistake in Fonero that was carried over from the Ripple codebase
      const tx = MagicSpoon.buildTxChangeTrust(driver.Server, this.account, {
        asset: new FoneroSdk.Asset(code, issuer),
      });
